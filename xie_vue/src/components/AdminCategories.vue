  ...
  <div class="p-6">
      ...
      <table class="w-full">
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="cat in categories" :key="cat.id" class="hover:bg-gray-50">
            ...
            <td class="p-4 text-gray-500 font-mono text-sm">{{ cat.slug }}</td>
            <td class="p-4">
              <span class="bg-blue-100 text-blue-800 px-2 py-1 rounded text-sm">{{ cat.products_count }} 件</span>
            </td>
            <td class="p-4 text-gray-500 text-sm max-w-xs truncate">{{ cat.description || '-' }}</td>
            <td class="p-4 text-center">
              <button @click="editCategory(cat)" class="text-blue-500 hover:text-blue-700 mr-3" title="編輯">
                <i class="fas fa-edit"></i>
              </button>
              <button @click="confirmDelete(cat)" class="text-red-500 hover:text-red-700" title="刪除">
    ...
    <div v-if="showAddModal || showEditModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
        ...
        <form @submit.prevent="showEditModal ? updateCategory() : createCategory()">
          <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 mb-1">分類名稱 *</label>
            <input v-model="form.name" type="text" required class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:border-xieOrange" placeholder="例如：飲料">
          </div>
          <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 mb-1">描述</label>
            <textarea v-model="form.description" rows="3" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:border-xieOrange" placeholder="分類說明（選填）"></textarea>
          </div>
          <div class="flex justify-end gap-2">
            <button type="button" @click="closeModal" class="px-4 py-2 text-gray-500 hover:bg-gray-100 rounded-lg">取消</button>
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
        ...
        <p v-if="deleteTarget?.products_count > 0" class="text-orange-600 bg-orange-50 p-3 rounded mb-4">
        ...
        <div v-if="deleteTarget?.products_count > 0" class="mb-4">
          <label class="flex items-center mb-2">
            ...
            <span class="text-sm text-gray-700">將商品轉移到其他分類</span>
          </label>
          <select v-if="reassignProducts" v-model="targetCategoryId" class="w-full border border-gray-300 rounded px-3 py-2">
        ...
        <div class="flex justify-end gap-2">
          <button @click="showDeleteModal = false" class="px-4 py-2 text-gray-500 hover:bg-gray-100 rounded-lg">取消</button>
