...
return [
    ...
    'description' => 'Mr. Xie\'s Convenience Store - 電商平台 RESTful API',

    // Text to place in the "Introduction" section, right after the `description`. Markdown and HTML are supported.
    'intro_text' => <<<INTRO
        歡迎使用謝老闆便利店 API。本文檔提供所有您需要的 API 使用資訊。

        ## 認證方式
        本 API 使用 **Laravel Sanctum** Token 認證。請在 Header 中加入：
        ```
        Authorization: Bearer {YOUR_TOKEN}
        ```

        ## 金額單位
        所有金額均以「分 (cents)」為單位（整數）。例如：`9900` = NT$99.00

        <aside>向右捲動可看到不同程式語言的範例程式碼。</aside>
    INTRO,
    ...
    'auth' => [
        ...
        // Options: 'bearer', 'basic', 'header', 'query', 'body'
        'in' => 'bearer',
        ...
        'extra_info' => '請透過 `POST /api/login` 獲取 Token。Token 有效期為 24 小時。',
    ],
